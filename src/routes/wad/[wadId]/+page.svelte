<script lang="ts">
  ...
  function trimToLength(maxLength: number, text?: string): string {
    text ??= '';
...
<style>
  .graphicsimage, .endoomthumb {
    width: 100%;
  }
  .automapimage, .endoomimage {
    max-width: 100%;
  }
...
<Container>
  ...
  <Col>
    ...
    <Row>
      <Col xs=2>Size</Col>
      <Col>{humanFileSize(Number(wad.Size))}</Col>
    </Row>
    <Row>
      <Col xs=2>MD5</Col>
  ...
  <Col xs=4 style="text-align: center">
    ...
    {#if wad.Description || wad.FallbackDescriptions?.[0]}
      {trimToLength(500, wad.Description ?? wad.FallbackDescriptions?.[0])}
    {:else}
      {wad.Filename ?? wad.FallbackFilenames[0]}
      {#if wad.Name || wad.FallbackNames[0]}
        ({wad.Name ?? wad.FallbackNames[0]})
      {/if}
      is {a_an(wad.Type)} {wad.Type} for {wad.Engines.join(', ')}
      {#if wad.Maps.length}
        featuring {wad.Maps.length} maps ({wad.Maps.map(e => e.NiceNames?.LevelName ?? e.FallbackNiceNames?.[0].LevelName ?? e.Name).join(', ')})
      {/if}
    {/if}
  </Col>
</Row>
...
<Container>
  <Row cols={{xs: 1, sm: 2}}>
    {#each wad.Maps as map}
      <Col id={map.Name}>
        <MapComponent wad={wad} map={map} openAutomapCallback={() => modalMap = map}/>
      </Col>
    {/each}
  </Row>
</Container>
